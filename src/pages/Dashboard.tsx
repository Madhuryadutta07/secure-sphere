...
const summaryMeta = [
  { label: "Total Customers", key: "totalCustomers" as const, change: "+12%", icon: Users, trend: "up" as const },
  { label: "Active Accounts", key: "activeAccounts" as const, change: "+8%", icon: Wallet, trend: "up" as const },
  { label: "Transactions", key: "transactionCount" as const, change: "+18%", icon: ArrowLeftRight, trend: "up" as const },
  { label: "Open Fraud Alerts", key: "openFraudAlerts" as const, change: "-5%", icon: AlertTriangle, trend: "down" as const },
];
...
const Dashboard = () => {
  ...
  const { data: fraudTrends = [] } = useQuery({ queryKey: ["trends", "fraud"], queryFn: api.getFraudTrends });
          ...
          {summaryMeta.map((card, i) => {
            const v =
              summary && !summaryLoading
                ? card.key === "openFraudAlerts"
                  ? String(summary.openFraudAlerts)
                  : formatKpi(summary[card.key])
                : "—";
            return (
